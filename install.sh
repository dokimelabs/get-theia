...
# The GitHub App client id the device grant runs against (a PUBLIC
...
# "theia releases" GitHub App: Contents read-only, installed on
# theia-releases alone, device flow enabled, token expiration OFF). A
# GitHub App token carries the app's fine-grained permission, never an
# OAuth scope — so no `scope` rides the request. THEIA_OAUTH_CLIENT_ID
# overrides for staging apps.
DEVICE_CLIENT_ID="${THEIA_OAUTH_CLIENT_ID:-Iv23liXy3IWQ9kVHGLGD}"
...
device_flow() {
  ...
  log "  ${vuri}    code: ${ucode}"
  log ""
  # mirrors SIGN_IN_ACCESS_NOTE in github-release-auth.ts: GitHub's page
  # names no repository and no permission, so the installer does
  log "  the GitHub page will say the app may \"act on your behalf\" — what it can"
  log "  actually reach is read-only release downloads from ${REPO};"
  log "  it cannot read or change anything in your own repositories."
  log ""
  waited=0
